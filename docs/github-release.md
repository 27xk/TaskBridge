...
这些 Secrets 是正式发布的推荐配置。缺少任一项时，Android 发布 job 默认会失败，不会回退到 debug signing。

如果只是需要先跑通发布流程，可以在仓库 Variables 中设置：

```text
TASKBRIDGE_ALLOW_UNSIGNED_RELEASE=true
```

这样 Release workflow 会在没有 Android 签名 Secrets 时产出 unsigned release APK。正式对外分发仍建议配置签名 Secrets，否则用户后续升级安装会受到签名一致性限制。
...
- release 签名 Secrets 是否完整；如果暂时不签名，仓库 Variables 是否设置了 `TASKBRIDGE_ALLOW_UNSIGNED_RELEASE=true`。
