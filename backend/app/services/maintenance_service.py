...
PRODUCT_EVENT_RETENTION_DAYS = 366
...
def cleanup_product_events(db: Session, *, user_id: int, now: datetime | None = None) -> int:
    current = now or utc_now()
    cutoff = current - timedelta(days=PRODUCT_EVENT_RETENTION_DAYS)
    result = db.execute(
